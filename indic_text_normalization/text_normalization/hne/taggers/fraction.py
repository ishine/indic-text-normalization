...
class FractionFst(GraphFst):
    ...
    def __init__(self, cardinal, deterministic: bool = True):
        ...
        dedh_dhai_graph = pynini.string_map(
            [("१" + NEMO_SPACE + CG_ONE_HALF, CG_DEDH), ("२" + NEMO_SPACE + CG_ONE_HALF, CG_DHAI)]
        )

        savva_numbers = cardinal_graph + pynini.cross(NEMO_SPACE + CG_ONE_QUARTER, "")
        savva_graph = pynutil.insert(CG_SAVVA) + pynutil.insert(NEMO_SPACE) + savva_numbers

        sadhe_numbers = cardinal_graph + pynini.cross(NEMO_SPACE + CG_ONE_HALF, "")
        ...
        paune_numbers = paune + pynini.cross(NEMO_SPACE + CG_THREE_QUARTERS, "")
