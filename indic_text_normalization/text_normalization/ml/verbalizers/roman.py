...
from indic_text_normalization.text_normalization.ml.graph_utils import NEMO_NOT_QUOTE, GraphFst
...
class RomanFst(GraphFst):
    ...
    Finite state transducer for verbalizing roman numerals
        e.g. tokens { roman { integer: "ഒന്ന്" } } -> ഒന്ന്
    ...
    def __init__(self, deterministic: bool = True):
        ...
        # Load ordinal mappings for Malayalam
        try:
            ...
            from indic_text_normalization.text_normalization.ml.graph_utils import NEMO_SIGMA
        ...
        graph |= (
            ...
            + pynini.accep(" ")
            + pynutil.delete("integer: \"")
            + pynini.closure(pynutil.insert(""), 0, 1)  # Malayalam doesn't use "the" prefix
            + ordinal
            + pynutil.delete("\"")
        ).optimize()
