...
class WordFst(GraphFst):
    """
    Finite state transducer for classifying Malayalam words.
        e.g. സോന ന -> tokens { name: "സോന ന" }
    ...
    def __init__(self, punctuation: PunctuationFst, deterministic: bool = True):
        ...
        # Define Malayalam characters and symbols using pynini.union
        HINDI_CHAR = pynini.union(
            *[chr(i) for i in range(0x0900, 0x0903 + 1)],  # Malayalam vowels and consonants
            *[chr(i) for i in range(0x0905, 0x0939 + 1)],  # More Malayalam characters
            *[chr(i) for i in range(0x093E, 0x094D + 1)],  # Malayalam diacritics
        ).optimize()
