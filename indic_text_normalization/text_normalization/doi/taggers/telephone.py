...
# Convert Arabic digits (0-9) to Dogri digits (०-९) for pattern matching
...
# Supports both Arabic digits (via digit_to_word) and Dogri digits (via digits)
num_token = pynini.union(digit_to_word, digits, zero).optimize()

# Pattern to match any digit (Arabic or Dogri) for telephone numbers
...
def generate_mobile(context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Support both Arabic and Dogri digits for mobile start digits
...
def get_landline(std_length: int, context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Landline number: match Arabic or Dogri digits, convert to words
...
def generate_credit(context_keywords: pynini.Fst) -> pynini.Fst:
    context_before, context_after = get_context(context_keywords)
    # Credit card: match Arabic or Dogri digits, convert to words digit-by-digit
...
def generate_pincode(context_keywords: pynini.Fst) -> pynini.Fst:
    context_before, context_after = get_context(context_keywords)
    # Pincode: match Arabic or Dogri digits, convert to words digit-by-digit
...
class TelephoneFst(GraphFst):
    ...
    Finite state transducer for tagging telephone numbers, e.g.
        ९१५७११४००७ -> telephone { number_part: "शून्य नौ इक्क पंज सत्त इक्क इक्क चार शून्य शून्य सत्त" }
        +९१ ९२१०५१५६०६ -> telephone { country_code: "प्लस नौ इक्क", number_part: "नौ दो इक्क शून्य पंज इक्क पंज छे शून्य छे" }
        १३७४-३०९९८८ -> telephone { number_part: "शून्य इक्क त्रे सत्त चार त्रे शून्य नौ नौ अठ्ठ अठ्ठ" }
