...
class TimeFst(GraphFst):
    ...
    Finite state transducer for classifying time, e.g.
        १२:३०:३०  -> time { hours: "बारां" minutes: "त्री" seconds: "त्री" }
        १:४०  -> time { hours: "इक्क" minutes: "चाली" }
        १:००  -> time { hours: "इक्क" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # Support both Dogri and Arabic digits for hours and minutes
        # For hours: delete leading zeros, then convert to Dogri if needed, then map to hours_graph
        # Dogri digits path: Dogri digits (with leading zero deletion) -> hours_graph
        dogri_hour_path = pynini.compose(
            delete_leading_zero_hindi,
            hours_graph
        ).optimize()
        # Arabic digits path: delete leading zero -> convert to Dogri -> hours_graph
        arabic_hour_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_dogri_number @ hours_graph
        ).optimize()
        hour_input = dogri_hour_path | arabic_hour_path

        # For minutes: same approach
        dogri_minute_path = pynini.compose(
            delete_leading_zero_hindi,
            minutes_graph
        ).optimize()
        arabic_minute_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_dogri_number @ minutes_graph
        ).optimize()
        minute_input = dogri_minute_path | arabic_minute_path

        # For seconds: same approach
        dogri_second_path = pynini.compose(
            delete_leading_zero_hindi,
            seconds_graph
        ).optimize()
        arabic_second_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_dogri_number @ seconds_graph
        ).optimize()
        second_input = dogri_second_path | arabic_second_path
        ...
        # hour - support both Dogri and Arabic double zero
