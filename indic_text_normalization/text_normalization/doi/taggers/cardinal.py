...
from indic_text_normalization.text_normalization.doi.graph_utils import GraphFst, NEMO_DIGIT, insert_space
...
class CardinalFst(GraphFst):
    ...
    Finite state transducer for classifying cardinals, e.g.
        -२३ -> cardinal { negative: "true"  integer: "त्रेई" }
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        # Support both Dogri digits and Arabic digits
        # Dogri digits go directly to final_graph
        dogri_final_graph = (
            digit
            | zero
            | teens_and_ties
        ...
        # Arabic digits: convert to Dogri, then apply the same graph
        arabic_digit_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_final_graph = pynini.compose(arabic_digit_input, arabic_to_dogri_number @ dogri_final_graph).optimize()

        # Combine both Dogri and Arabic digit paths
        final_graph = dogri_final_graph | arabic_final_graph
