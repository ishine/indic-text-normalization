...
class MeasureFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst, fraction: GraphFst = None, deterministic: bool = True):
        ...
        # Support both Malayalam and Arabic digits
        ...
        # Arabic digits path - convert to Malayalam first, then compose with cardinal
        # Malayalam number input
        ...
        # Arabic number input - convert to Malayalam, then compose with cardinal
        ...
        # Combined cardinal graph (supports both Malayalam and Arabic digits)
        ...
        # Note: Malayalam units may not have distinct plural forms, but keeping structure similar
        unit_singular = (
            pynutil.insert('units: "') + (unit_graph + optional_graph_unit2 | graph_unit2) + pynutil.insert('"')
        )
        unit_plural = unit_singular  # Malayalam units typically don't change for plural
        ...
        # This uses cardinal_graph_base which already supports both Malayalam and Arabic digits
        graph_cardinal = (
            pynutil.insert("cardinal { ")
            + optional_graph_negative
            + pynutil.insert("integer: \"")
