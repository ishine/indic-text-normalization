...
# Supports both Arabic digits (via digit_to_word) and Malayalam digits (via digits)
...
def generate_mobile(context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Support both Arabic and Malayalam digits for mobile start digits
    mobile_start_digit = pynini.union(
        HI_MOBILE_START_DIGITS @ digits, 
        HI_MOBILE_START_DIGITS @ digit_to_word
    )

    # Country code: match Arabic or Malayalam digits, then convert to words
    # Accept 1-3 digits (Arabic or Malayalam), convert to words digit-by-digit
    ...
    # Number part: match Arabic or Malayalam digits, then convert to words digit-by-digit
...
def get_landline(std_length: int, context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Landline number: match Arabic or Malayalam digits, convert to words
...
def generate_credit(context_keywords: pynini.Fst) -> pynini.Fst:
    context_before, context_after = get_context(context_keywords)
    # Credit card: match Arabic or Malayalam digits, convert to words digit-by-digit
...
def generate_pincode(context_keywords: pynini.Fst) -> pynini.Fst:
    context_before, context_after = get_context(context_keywords)
    # Pincode: match Arabic or Malayalam digits, convert to words digit-by-digit
