...
CG_LANDLINE_START_DIGITS = pynini.union("२", "३", "४", "६", "2", "3", "4", "6").optimize()
...
def generate_mobile(context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    mobile_start_digit = pynini.union(CG_MOBILE_START_DIGITS @ digits, CG_MOBILE_START_DIGITS @ digit_to_word)
...
def get_landline(std_length: int, context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Filter cardinals to only include allowed digits
    landline_start_digit = pynini.union(CG_LANDLINE_START_DIGITS @ digits, CG_LANDLINE_START_DIGITS @ digit_to_word)
...
def get_context(keywords: pynini.Fst):
    ...
    all_digits = pynini.union(NEMO_CG_DIGIT, NEMO_DIGIT)
