...
from nemo_text_processing.text_normalization.ne.graph_utils import GraphFst, NEMO_DIGIT, insert_space
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        # Hundred graph
        suffix_hundreds = pynutil.insert(" सय")
        ...
        suffix_crores = pynutil.insert(" करोड")
        ...
        suffix_arabs = pynutil.insert(" अर्ब")
        ...
        suffix_kharabs = pynutil.insert(" खर्ब")
        ...
        # Support both Nepali digits and Arabic digits
        # Nepali digits go directly to final_graph
        Nepali_final_graph = (
            digit
            | zero
            | teens_and_ties
        ...
        # Arabic digits: convert to Nepali, then apply the same graph
        arabic_digit_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_final_graph = pynini.compose(arabic_digit_input, arabic_to_Nepali_number @ Nepali_final_graph).optimize()

        # Combine both Nepali and Arabic digit paths
        final_graph = Nepali_final_graph | arabic_final_graph
