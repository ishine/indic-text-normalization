...
class WordFst(GraphFst):
    """
    Finite state transducer for classifying Nepali words.
    ...
    def __init__(self, punctuation: PunctuationFst, deterministic: bool = True):
        ...
        # Define Nepali characters and symbols using pynini.union
        HINDI_CHAR = pynini.union(
            *[chr(i) for i in range(0x0900, 0x0903 + 1)],  # Nepali vowels and consonants
            *[chr(i) for i in range(0x0905, 0x0939 + 1)],  # More Nepali characters
            *[chr(i) for i in range(0x093E, 0x094D + 1)],  # Nepali diacritics
        ).optimize()
