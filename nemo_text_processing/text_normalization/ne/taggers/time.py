...
class TimeFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # For Nepali: same pattern with Nepali digits
        delete_leading_zero_Nepali = (
            # Match two Nepali digits
            NEMO_HI_DIGIT + NEMO_HI_DIGIT
        ) | (
            # Match optional leading "०" (0 or 1 times) + Nepali digit
            pynini.closure(pynutil.delete("०"), 0, 1) + NEMO_HI_DIGIT
        )

        # Support both Nepali and Arabic digits for hours and minutes
        # For hours: delete leading zeros, then convert to Nepali if needed, then map to hours_graph
        # Nepali digits path: Nepali digits (with leading zero deletion) -> hours_graph
        Nepali_hour_path = pynini.compose(
            delete_leading_zero_Nepali,
            hours_graph
        ).optimize()
        # Arabic digits path: delete leading zero -> convert to Nepali -> hours_graph
        arabic_hour_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_Nepali_number @ hours_graph
        ).optimize()
        hour_input = Nepali_hour_path | arabic_hour_path

        # For minutes: same approach
        Nepali_minute_path = pynini.compose(
            delete_leading_zero_Nepali,
            minutes_graph
        ).optimize()
        arabic_minute_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_Nepali_number @ minutes_graph
        ).optimize()
        minute_input = Nepali_minute_path | arabic_minute_path

        # For seconds: same approach
        Nepali_second_path = pynini.compose(
            delete_leading_zero_Nepali,
            seconds_graph
        ).optimize()
        arabic_second_path = pynini.compose(
            delete_leading_zero_arabic,
            arabic_to_Nepali_number @ seconds_graph
        ).optimize()
        second_input = Nepali_second_path | arabic_second_path
        ...
        # hour - support both Nepali and Arabic double zero
        Nepali_double_zero = pynutil.delete(HI_DOUBLE_ZERO)
        arabic_double_zero = pynutil.delete("00")
        double_zero = Nepali_double_zero | arabic_double_zero
        graph_h = self.hours + delete_colon + double_zero

        # Support both Nepali and Arabic time patterns for dedh/dhai
        dedh_dhai_graph = (
        ...
        # Support both Nepali and Arabic time patterns
        savva_numbers = (
