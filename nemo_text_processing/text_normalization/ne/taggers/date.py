...
class DateFst(GraphFst):
    ...
    Finite state transducer for classifying date, e.g.
        ...
        "०४-०१-२०२४" -> date { month: "अप्रैल" day: "एक" year: "दो हजार चौबीस" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # Support both Nepali and Arabic digits for dates
        ...
        # Nepali digits for year patterns
        Nepali_year_thousands = pynini.compose(
            year_pattern_thousands, cardinal.graph_thousands
        )
        Nepali_year_hundreds_as_thousands = pynini.compose(
            year_pattern_hundreds, cardinal.graph_hundreds_as_thousand
        )
        
        # Arabic digits for year patterns - convert to Nepali first
        # Convert 4-digit Arabic year (e.g., "2024") to Nepali ("२०२४"), then match patterns
        arabic_year_4digits = (NEMO_DIGIT + NEMO_DIGIT + NEMO_DIGIT + NEMO_DIGIT)
        # Convert Arabic to Nepali
        arabic_to_Nepali_year = arabic_year_4digits @ arabic_to_Nepali_number
        
        # Match converted Nepali year against patterns and compose with cardinal
        arabic_year_thousands = pynini.compose(
            arabic_to_Nepali_year,
        ...
        # Combined year graphs (supports both Nepali and Arabic digits)
        graph_year_thousands = Nepali_year_thousands | arabic_year_thousands
        graph_year_hundreds_as_thousands = Nepali_year_hundreds_as_thousands | arabic_year_hundreds_as_thousands
        ...
        # Support both Nepali and Arabic digits for days and months
        # Nepali digits path: Nepali digits -> days/months mapping
        Nepali_days_graph = pynutil.insert("day: \"") + days + pynutil.insert("\"") + insert_space
        Nepali_months_graph = pynutil.insert("month: \"") + months + pynutil.insert("\"") + insert_space
        
        # Arabic digits path: Arabic digits -> convert to Nepali -> days/months mapping
        ...
        arabic_months_graph = pynutil.insert("month: \"") + pynini.compose(
            arabic_month_input,
            arabic_to_Nepali_number @ months
        ) + pynutil.insert("\"") + insert_space
        
        # Combined graphs (supports both Nepali and Arabic digits)
        days_graph = Nepali_days_graph | arabic_days_graph
        months_graph = Nepali_months_graph | arabic_months_graph
        ...
        # Graph for year - support both Nepali and Arabic digits
        # Nepali digits path
        Nepali_century_input = pynini.closure(NEMO_HI_DIGIT, 1)
        Nepali_century_number = pynini.compose(Nepali_century_input, cardinal_graph) + pynini.accep("वीं")
        ...
        century_number = Nepali_century_number | arabic_century_number
