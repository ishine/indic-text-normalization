...
class DecimalFst(GraphFst):
    ...
    Finite state transducer for classifying decimal, e.g.
        -१२.५००६ अर्ब -> decimal { negative: "true" integer_part: "बारह"  fractional_part: "पाँच शून्य शून्य छह" quantity: "अरब" }
        १ अर्ब -> decimal { integer_part: "एक" quantity: "अरब" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Support both Nepali and Arabic digits for fractional part
        # Nepali digits path: Nepali digits -> cardinal digit/zero mapping
        Nepali_digit_graph = cardinal.digit | cardinal.zero
        Nepali_fractional_input = pynini.closure(NEMO_HI_DIGIT, 1)
        Nepali_fractional_graph = pynini.compose(Nepali_fractional_input, Nepali_digit_graph).optimize()
        
        # Arabic digits path: Arabic digits -> convert to Nepali -> cardinal digit/zero mapping
        arabic_fractional_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_fractional_graph = pynini.compose(
            arabic_fractional_input,
            arabic_to_Nepali_number @ Nepali_digit_graph
        ).optimize()
        
        # Combined fractional digit graph (supports both Nepali and Arabic digits)
        graph_digit = Nepali_fractional_graph | arabic_fractional_graph
        ...
        # Support both Nepali and Arabic digits for integer part
        cardinal_graph = cardinal.final_graph
        
        # Nepali digits input for integer part
        Nepali_integer_input = pynini.closure(NEMO_HI_DIGIT, 1)
        Nepali_integer_graph = pynini.compose(Nepali_integer_input, cardinal_graph).optimize()
        ...
        # Combined integer graph (supports both Nepali and Arabic digits)
        integer_graph = Nepali_integer_graph | arabic_integer_graph
