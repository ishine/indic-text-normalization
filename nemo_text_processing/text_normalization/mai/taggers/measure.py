...
MAI_POINT_FIVE = ".५"  # .5
MAI_ONE_POINT_FIVE = "१.५"  # 1.5
MAI_TWO_POINT_FIVE = "२.५"  # 2.5
MAI_DECIMAL_25 = ".२५"  # .25
MAI_DECIMAL_75 = ".७५"  # .75
...
class MeasureFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst, fraction: GraphFst = None, deterministic: bool = True):
        ...
        # Support both Maithili and Arabic digits
        # Maithili digits path
        maithili_cardinal_graph_base = (
            cardinal.zero
            | cardinal.digit
        ...
        # Maithili number input
        maithili_number_input = pynini.closure(NEMO_MAI_DIGIT, 1)
        maithili_number_graph = pynini.compose(maithili_number_input, maithili_cardinal_graph_base).optimize()
        
        # Arabic number input - convert to Maithili, then compose with cardinal
        arabic_number_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_number_graph = pynini.compose(
            arabic_number_input,
            arabic_to_maithili_number @ maithili_cardinal_graph_base
        ).optimize()
        
        # Combined cardinal graph (supports both Maithili and Arabic digits)
        cardinal_graph_base = maithili_number_graph | arabic_number_graph
        ...
        quarter = pynini.string_map(
            [
                (MAI_POINT_FIVE, MAI_SADHE),
                (MAI_ONE_POINT_FIVE, MAI_DEDH),
                (MAI_TWO_POINT_FIVE, MAI_DHAI),
            ]
        )
        ...
        # Note: Maithili units may not have distinct plural forms, but keeping structure similar
        unit_singular = (
            pynutil.insert('units: "') + (unit_graph + optional_graph_unit2 | graph_unit2) + pynutil.insert('"')
        )
        unit_plural = unit_singular  # Maithili units typically don't change for plural
        ...
        dedh_dhai = pynini.string_map([(MAI_ONE_POINT_FIVE, MAI_DEDH), (MAI_TWO_POINT_FIVE, MAI_DHAI)])
        dedh_dhai_graph = pynutil.insert("integer: \"") + dedh_dhai + pynutil.insert("\"")

        savva_numbers = cardinal_graph_base + pynini.cross(MAI_DECIMAL_25, "")
        savva_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAI_SAVVA)
            + pynutil.insert(NEMO_SPACE)
            + savva_numbers
            + pynutil.insert("\"")
        )

        sadhe_numbers = cardinal_graph_base + pynini.cross(MAI_POINT_FIVE, "")
        sadhe_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAI_SADHE)
            + pynutil.insert(NEMO_SPACE)
            + sadhe_numbers
            + pynutil.insert("\"")
        )
        ...
        paune_numbers = paune + pynini.cross(MAI_DECIMAL_75, "")
        paune_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAI_PAUNE)
            + pynutil.insert(NEMO_SPACE)
            + paune_numbers
            + pynutil.insert("\"")
        )
        ...
        # Note: cardinal_graph already handles both Maithili and Arabic digits
        ...
        # Handle both ASCII "1" and Maithili "१"
        ...
        # This uses cardinal_graph_base which already supports both Maithili and Arabic digits
        graph_cardinal = (
            pynutil.insert("cardinal { ")
            + optional_graph_negative
            + pynutil.insert("integer: \"")
        ...
        # Note: Maithili has math_operations.tsv in data/ folder (not data/measure/)
