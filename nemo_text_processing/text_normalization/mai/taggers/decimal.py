...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Support both Maithili and Arabic digits for fractional part
        # Maithili digits path: Maithili digits -> cardinal digit/zero mapping
        maithili_digit_graph = cardinal.digit | cardinal.zero
        maithili_fractional_input = pynini.closure(NEMO_MAI_DIGIT, 1)
        maithili_fractional_graph = pynini.compose(maithili_fractional_input, maithili_digit_graph).optimize()
        
        # Arabic digits path: Arabic digits -> convert to Maithili -> cardinal digit/zero mapping
        arabic_fractional_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_fractional_graph = pynini.compose(
            arabic_fractional_input,
            arabic_to_maithili_number @ maithili_digit_graph
        ).optimize()
        
        # Combined fractional digit graph (supports both Maithili and Arabic digits)
        graph_digit = maithili_fractional_graph | arabic_fractional_graph
        ...
        # Support both Maithili and Arabic digits for integer part
        cardinal_graph = cardinal.final_graph
        
        # Maithili digits input for integer part
        maithili_integer_input = pynini.closure(NEMO_MAI_DIGIT, 1)
        maithili_integer_graph = pynini.compose(maithili_integer_input, cardinal_graph).optimize()
        ...
        # Combined integer graph (supports both Maithili and Arabic digits)
        integer_graph = maithili_integer_graph | arabic_integer_graph
